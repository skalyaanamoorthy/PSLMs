...
We recommend demoing analysis_notebooks/k2369_analysis_demo.ipynb; the other notebooks are fully functional and show how other figures are generated but will take a long time to run due to the number of bootstrap replicates. The expected outputs are shown below the cells. The expected runtimes are often included per-cell, for a total runtime of <5 minutes on a typical PC. To reduce the demo time, the number of bootstrapped replicates have been greatly reduced from those reported in the text. Additionally, the ensemble predictions step has been precomputed and is loaded from a file, but can be easily reproduced by uncommenting the relevant line(s).
...
ℹ️ ✔️ **You can now proceed directly to run the demo analysis_notebooks/k2369_analysis_demo.ipynb if you are demoing analysis only.**
...
*If using Docker: make sure your virtual environment is activated `source /app/venv/bin/activate` and you are in the correct directory `cd /app`. However, this should be automatically when launching the instance.*
