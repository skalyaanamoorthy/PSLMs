...
def score_sequences(args):
    ...
    logps = pd.DataFrame(index=df2.index, columns=['esm2_15B_half_dir', 'runtime_esm2_15B_half_dir'])
            ...
            for uid, row in group.iterrows():
                with torch.no_grad():
                    try:
                        ...
                        logps.at[uid, f'esm2_15B_half_dir'] = score.item()
                        logps.at[uid, f'runtime_esm2_15B_half_dir'] = time.time() - start
                    except:
                        print('failed: ', code, wt, pos, mt)
                        logps.at[uid, f'esm2_15B_half_dir'] = np.nan
                        logps.at[uid, f'runtime_esm2_15B_half_dir'] = np.nan
                    pbar.update(1)
    
    df = pd.read_csv(args.output, index_col=0)
    logps.index.name = 'uid'
    df = pd.read_csv(args.output, index_col=0)
    if 'esm2_15B_half_dir'in df.columns:
        df = df.drop(f'esm2_15B_half_dir', axis=1)
    if 'runtime_esm2_15B_half_dir' in df.columns:
        df = df.drop(f'runtime_esm2_15B_half_dir', axis=1)
    df = df.join(logps)
...
def main():
    parser = argparse.ArgumentParser(
            description='Score sequences based on a given sequence.'
    )
    parser.add_argument(
            '--db_loc', type=str,
