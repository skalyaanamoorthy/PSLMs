...
def score_sequences(args):
    ...
    logps = pd.DataFrame(index=df2.index, columns=['esm2_650M_dir', 'runtime_esm2_650M_dir'])
    ...
    model, alphabet = pretrained.load_model_and_alphabet(f'esm2_t33_650M_UR50D')
                ...
                with torch.no_grad():
                    ...
                    #try:
                        ...
                        try:
                            ...
                            print('input sequence does not match designate wild-type. ', code, wt, pos, mt)
    ...
    df = pd.read_csv(args.output, index_col=0)
    if f'esm2_650M_dir' in df.columns:
        df = df.drop(f'esm2_650M_dir', axis=1)
    if f'runtime_esm2_650M_dir' in df.columns:
        df = df.drop(f'runtime_esm2_650M_dir', axis=1)
    df = df.join(logps)
