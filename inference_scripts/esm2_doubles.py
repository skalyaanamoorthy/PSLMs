...
def score_sequences(args):
    ...
    logps = pd.DataFrame(index=df2.index,columns=[args.model, f'runtime_{args.model}_dir'])

    if True:
        model, alphabet = pretrained.load_model_and_alphabet(args.model_id)
        model.eval()
                ...
                for uid, row in group.iterrows():
                    with torch.no_grad():
                        ...
                        try:
                            ...
                            logps.at[uid, f'{args.model}_dir'] = score.item()
                            logps.at[uid, f'runtime_{args.model}_dir'] = time.time() - start
                        except:
                            print(code, wt1, pos1, mt1, wt2, pos2, mt2)
                            logps.at[uid, f'{args.model}_dir'] = np.nan
                            logps.at[uid, f'runtime_{args.model}_dir'] = np.nan
                        pbar.update(1)
...
def main():
    ...
    parser.add_argument(
            '--model', default='esm2_15B'
    )
    args = parser.parse_args()
    
    model_lookup = {'esm2_15B': 'esm2_t48_15B_UR50D', 'esm2_3B': 'esm2_t36_3B_UR50D', 'esm2_650M': 'esm2_t33_650M_UR50D', 'esm2_150M': 'esm2_t30_150M_UR50D'}
    args.model_id = model_lookup[args.model]
