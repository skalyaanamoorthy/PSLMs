        ...
        for (code, chain), group in df2.groupby(['code', 'chain']):
            ...
            with torch.no_grad():
                    for uid, row in group.iterrows():
                        ...
                        oc = -1#int(ou) * (1 if args.dataset == 'fireprot' else 0)  -1 
                        ...
                        logps.at[uid, f'mpnn_masked_{args.noise}_{str(args.bbnoise).replace(".", "")}_dir'] = (log_probs_native[0][pos+oc][alphabet.find(mut)] -\
                            log_probs_native[0][pos+oc][alphabet.find(wt)]).item()
                        logps.at[uid, f'runtime_mpnn_masked_{args.noise}_{str(args.bbnoise).replace(".", "")}_dir'] = time.time() - start
                        pbar.update(1)

    logps.index.name = 'uid'
    df = pd.read_csv(args.output, index_col=0)
    if f'mpnn_masked_{args.noise}_{str(args.bbnoise).replace(".", "")}_dir' in df.columns:
        df = df.drop(f'mpnn_masked_{args.noise}_{str(args.bbnoise).replace(".", "")}_dir', axis=1)
    if f'runtime_mpnn_masked_{args.noise}_{str(args.bbnoise).replace(".", "")}_dir' in df.columns:
        df = df.drop(f'runtime_mpnn_masked_{args.noise}_{str(args.bbnoise).replace(".", "")}_dir', axis=1)
    df = df.join(logps)
